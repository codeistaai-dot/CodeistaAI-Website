...
export class UserService {
  ...
  async registerUser(data: z.infer<typeof SignupSchema>, clientIp?: string, userAgent?: string) {
    ...
    if (existingUser) {
      ...
      await this.repository.updateExistingUser(existingUser._id as any, userData, campaignData, courseEnquiryData);
      
      // Return the updated user object for the response
      const updatedUser = { ...existingUser.toObject(), name: userData.name, pythonStartingPoint: courseEnquiryData.pythonStartingPoint, message: courseEnquiryData.message };
